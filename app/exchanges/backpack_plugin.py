"""
app/exchanges/backpack_plugin.py — Backpack Exchange Plugin
=============================================================
ccxt.pro WebSocket feed for Backpack spot orderbooks (USDC quote).
"""
...
class BackpackPlugin(ExchangePlugin):
    """Backpack exchange via ccxt.pro WebSocket."""

    def __init__(self, config: ExchangeConfig):
        super().__init__(config)
        self._exchange: Optional[ccxt.backpack] = None
    ...
    async def connect(self) -> None:
        ...
        self._exchange = ccxt.backpack(ccxt_config)
    ...
    async def place_buy(self, symbol: str, qty: float, price: float, **params) -> dict:
        if not self._exchange:
            raise RuntimeError("Backpack not connected")
