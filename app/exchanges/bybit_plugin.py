"""
app/exchanges/bybit_plugin.py — Bybit Exchange Plugin
=======================================================
ccxt.pro WebSocket feed for Bybit spot orderbooks.
Note: Bybit spot only accepts ob_limit of [1, 50, 200, 1000].
"""
...
class BybitPlugin(ExchangePlugin):
    """Bybit exchange via ccxt.pro WebSocket."""
    ...
    async def watch_orderbook(self, token: str, symbol: str) -> Optional[dict]:
        if not self._exchange:
            return None
        ob_limit = self.config.ob_limit or 50
